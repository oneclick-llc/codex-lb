...
Two fixes to the existing opportunistic gate ship alongside the mode and apply whether or not it is enabled: a `preserve` account for which upstream reports no 5h window is gated by its weekly floor alone instead of being refused all opportunistic burn, and a closed burn window now answers with a `429` rate-limit error on every path — the WebSocket and first-turn paths previously fell through to a `503` that Codex reports as "unexpected status".
