...
- **GIVEN** an over-share key and a `normal` account whose 5h window is not reported (`reset_at` unknown), 3 days from its weekly reset (pace line 43% minus 10pp slack = 33%) and with 30% remaining
- **WHEN** the key submits a request
