...
#### Scenario: Pool outage passes through the admission gate

- **GIVEN** an over-share key and a pool whose accounts are all in upstream error backoff
- **WHEN** the HTTP admission preflight consults the opportunistic gate and gets a code-less pool-level failure
- **THEN** the preflight does not deny; the request proceeds to normal selection and fails with the same pool-level error foreground traffic receives
